...
public class XMLParser {
    ...
    public XMLParser(String rootElementName, boolean parserWarnings, boolean strictXML) {
    ...
    public XMLParser(String rootElementName, boolean verbose, boolean parserWarnings, boolean strictXML) {
        this(rootElementName, parserWarnings, strictXML);
     ...
     * @throws beast.xml.XMLParseException
     * @throws javax.xml.parsers.ParserConfigurationException
     *
     */
    public <T> T parse(Reader reader, Class<T> target)
            throws java.io.IOException,
            org.xml.sax.SAXException,
            beast.xml.XMLParseException,
        ...
        if (e.getTagName().equals(rootElementName)) {

            concurrent = false;
            return convert(e, target, false, true);

        } else {
            throw new beast.xml.XMLParseException("Unknown root document element, " + e.getTagName());
        }
    }
...
//            System.out.println("\tSystem ID: " + e.getSystemId());
    ...
    private <T> T convert(Element e, Class<T> target, boolean run, boolean doParse) throws XMLParseException {
...
//                    System.err.println("WARNING: " + msg);
                    java.util.logging.Logger.getLogger("beast.xml").warning(msg);
                }
            }
            ...
            return (T) new Reference(restoredXMLObject);
            ...
            if (parser != null) {
                ...
                for (final Map.Entry<Class,Collection> entry : globalElementCollections.entrySet()) {
                    if (entry.getKey().isInstance(obj)) entry.getValue().add(obj);
                }
...
//        if (xo.hasAttribute(MCLogger.PARSER.ALLOW_OVERWRITE_LOG)) {
//            allowOverwrite = xo.getBooleanAttribute(MCLogger.PARSER.ALLOW_OVERWRITE_LOG);
//        }
    ...
    public <T> void addGlobalElementCollection(final Class<T> c, final Collection<T> collection) {
        globalElementCollections.put(c, collection);
    }
    ...
    private final Map<Class,Collection> globalElementCollections = new HashMap<>();
