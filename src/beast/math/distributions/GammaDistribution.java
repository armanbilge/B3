...
//        }
    ...
    public static void main(String[] args) {
        ...
        for (double i = 0.0125; i < 1.0; i += 0.025) {
        	System.out.print(i + ": ");
        	try {
        		System.out.println(new org.apache.commons.math3.distribution.GammaDistribution(0.878328435043444, 0.0013696236839573005).inverseCumulativeProbability(i));
        	} catch (Exception e) {
...
//        test(0.9, 1.0, iters);
    ...
    private static void testQuantileCM(double y, double shape, double scale) {
        ...
        try {
            for (int i = 0; i < 1000; i++) {
                value = (new org.apache.commons.math3.distribution.GammaDistribution(shape, scale)).inverseCumulativeProbability(y);
            }
            value = (new org.apache.commons.math3.distribution.GammaDistribution(shape, scale)).inverseCumulativeProbability(y);
        } catch (Exception e) {
            e.printStackTrace();
